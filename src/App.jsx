...
import { Plus, Calendar as CalendarIcon, Tag, Mic, Sun, Moon, CheckCircle2, Filter, Save, Upload, Trash2, Repeat, Bell, ChevronDown, ChevronRight, Search, Users, Award } from "lucide-react";
import * as chrono from "chrono-node";
import { format, startOfWeek, addDays, isSameDay, startOfDay, endOfDay, parseISO, isBefore, addWeeks, addMonths, addDays as dfAddDays, isToday } from "date-fns";
import { ResponsiveContainer, LineChart, Line, XAxis, YAxis, Tooltip, CartesianGrid, RadialBarChart, RadialBar, PolarAngleAxis } from "recharts";
import { DragDropContext, Droppable, Draggable } from "@hello-pangea/dnd";

// --- Utilities ---------------------------------------------------------------
const priorities = ["Low", "Medium", "High"];
const defaultProjects = ["Inbox", "Work", "Personal", "School"]; const areas = ["Personal", "Work"];

function uid() { return Math.random().toString(36).slice(2) + Date.now().toString(36); }

function parseQuickInput(text) {
  // Extract tags (#tag), priority (!! or !high), project (p:Work), "every monday" style repeat, and due datetime via chrono
  ...
  // repeat: every monday|daily|weekly|monthly
  ...
  // due via chrono (ignore words we used)
  const cleaned = text
...
function nextOccurrence(task) {
  ...
  if (task.repeat.type === "daily") return dfAddDays(due, 1);
  if (task.repeat.type === "weekly") {
    if (task.repeat.weekday) {
      const map = { sunday: 0, monday: 1, tuesday: 2, wednesday: 3, thursday: 4, friday: 5, saturday: 6 };
      const target = map[task.repeat.weekday] ?? due.getDay();
      let d = new Date(due);
      do { d = dfAddDays(d, 1); } while (d.getDay() !== target);
      return d;
    }
    return addWeeks(due, 1);
  }
  if (task.repeat.type === "monthly") return addMonths(due, 1);
  return null;
}
...
function scheduleNotification(task) {
  try {
    ...
    if (delay <= 0 || delay > 1000 * 60 * 60 * 24) return; // schedule up to 24h ahead
    setTimeout(() => {
      new Notification("Reminder: " + task.title, { body: task.project + (task.tags?.length ? " • #" + task.tags.join(" #") : "") });
    }, delay);
  } catch {}
}
...
// --- State ------------------------------------------------------------------
const initial = () => {
  const raw = localStorage.getItem("todoapp_v3");
...
function reducer(state, action) {
  switch (action.type) {
    case "ADD_TASK": {
      const t = { id: uid(), createdAt: Date.now(), completed: false, subtasks: [], reminder: 30, ...action.payload };
      scheduleNotification(t);
      return { ...state, tasks: [t, ...state.tasks] };
    }
    case "UPDATE_TASK": {
      const tasks = state.tasks.map(t => t.id === action.id ? { ...t, ...action.payload } : t);
    ...
    case "TOGGLE_COMPLETE": {
      const tasks = state.tasks.map(t => t.id === action.id ? { ...t, completed: !t.completed, completedAt: !t.completed ? Date.now() : null } : t);
      ...
      if (task && !task.completed) {
        ...
        if (idx >= 0) history[idx] = { ...history[idx], completed: history[idx].completed + 1 };
        else history.push({ date: todayKey, completed: 1 });
        // handle recurring
        if (task.repeat) {
          const next = nextOccurrence(task);
          if (next) {
            const newTask = { ...task, id: uid(), completed: false, completedAt: null, createdAt: Date.now(), due: next };
            tasks.unshift(newTask);
            scheduleNotification(newTask);
          }
        }
      }
      return { ...state, tasks, points, history };
    }
    case "ADD_PROJECT": {
...
export default function ProTodoApp() {
  ...
  useEffect(() => { localStorage.setItem("todoapp_v3", JSON.stringify(state)); }, [state]);
  ...
  useEffect(() => {
    const onKey = (e) => {
      ...
      if (e.key === '/') { e.preventDefault(); const el = document.getElementById('search'); el?.focus(); }
  ...
  function addTaskFromQuick() {
    ...
    dispatch({ type: 'ADD_TASK', payload: { title: meta.title, tags: meta.tags, project: meta.project, priority: meta.priority, due, repeat: meta.repeat, area: quickArea } });
    setQuick("");
  }
  ...
  return (
    <div className="min-h-screen bg-[#F4F1EA] text-[#3E3E3B] dark:bg-[#11140F] dark:text-[#ECEBE6]">
      <header className="sticky top-0 z-20 backdrop-blur bg-[#FBF9F3]/60 border-b border-[#E7E2D6]"><div className="max-w-6xl mx-auto p-3 flex items-center gap-2"><h1 className="text-2xl font-semibold">Today</h1><div className="ml-auto flex items-center gap-2"><button onClick={() => dispatch({ type: 'TOGGLE_DARK' })} className="px-3 py-1 rounded-xl border border-[#E7E2D6] hover:bg-[#F7F3EA] flex items-center gap-2"><span className="hidden sm:inline">Theme</span></button><ImportExport state={state} dispatch={dispatch} /></div></div></header>

{/* Week strip */}
<div className="max-w-6xl mx-auto px-3 mt-2">
  <div className="mt-2 flex justify-between">
    {weekDays.map((d, idx) => (
      <button key={idx} type="button" onClick={()=>setSelectedDay(d)} className="flex flex-col items-center text-xs focus:outline-none">
        ...
        <section className="lg:col-span-2 space-y-3">
          <div className="p-3 rounded-2xl border border-[#E7E2D6] dark:border-[#2A2E25] bg-[#FBF9F3] dark:bg-[#191D16] shadow-sm">
            <div className="flex items-center gap-2">
              <input ref={inputRef} value={quick} onChange={e => setQuick(e.target.value)} onKeyDown={e => e.key==='Enter' && addTaskFromQuick()} placeholder='Quick add: "Buy milk tomorrow 5pm #groceries p:Personal !! every monday"' className="flex-1 bg-transparent outline-none px-3 py-2 rounded-xl border border-[#E7E2D6]"/>
              <select value={quickArea} onChange={e=>setQuickArea(e.target.value)} className="px-2 py-2 rounded-xl border border-[#E7E2D6] bg-transparent">
                {areas.map(a=> <option key={a} value={a}>{a}</option>)}
              </select>
              <button onClick={addTaskFromQuick} className="px-3 py-2 rounded-xl bg-[#9AA27A] text-white hover:opacity-90 flex items-center gap-2"><Plus className="w-4 h-4"/>Add</button>
              <button onClick={onVoice} className={`px-3 py-2 rounded-xl border border-[#E7E2D6] dark:border-[#2A2E25] hover:bg-[#F7F3EA] dark:hover:bg-[#20251D] ${voiceListening ? 'animate-pulse' : ''}`} title="Voice input"><Mic className="w-4 h-4"/></button>
            </div>
            <p className="text-xs text-[#8E8B80] mt-2">Shortcuts: <kbd className="px-1 border rounded">n</kbd> new • <kbd className="px-1 border rounded">/</kbd> search • <kbd className="px-1 border rounded">Shift+D</kbd> theme</p>
          </div>

          <div className="p-3 rounded-2xl border border-[#E7E2D6] dark:border-[#2A2E25] bg-[#FBF9F3] dark:bg-[#191D16] shadow-sm">
            <div className="flex items-center gap-2 mb-3"><Search className="w-4 h-4"/><input id="search" value={query} onChange={e=>setQuery(e.target.value)} placeholder='Search e.g. tag:urgent AND due:today project:Work priority:High' className="flex-1 bg-transparent outline-none px-3 py-2 rounded-xl border border-[#E7E2D6]"/></div>
            <div className="flex items-center justify-between mt-1 mb-2"><div className="text-xl font-medium">To Do List</div><button className="w-8 h-8 rounded-full bg-[#9AA27A] text-white flex items-center justify-center">+</button></div>
            <TaskList tasks={filtered} state={state} dispatch={dispatch} />
          </div>
        </section>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 xl:grid-cols-7 gap-3">
              {weekDays.map((d, idx) => (
                <Droppable droppableId={`day-${idx}`} key={idx}>
                  {(provided) => (
                    <div ref={provided.innerRef} {...provided.droppableProps} className="p-3 rounded-xl border border-[#E7E2D6] dark:border-[#2A2E25] bg-[#FBF9F3] dark:bg-[#191D16] min-h-[160px]">
                      <div className="text-xs text-[#8E8B80] mb-2 font-medium">{format(d, 'EEE dd')}</div>
                      {state.tasks.filter(t => t.due && isSameDay(new Date(t.due), d)).sort((a,b)=> new Date(a.due)-new Date(b.due)).map((t, i) => (
                        <Draggable draggableId={t.id} index={i} key={t.id}>
                          {(prov) => (
                            <div ref={prov.innerRef} {...prov.draggableProps} {...prov.dragHandleProps} className={`mb-2 p-2 rounded-lg text-sm ${t.completed ? 'line-through opacity-60' : ''} border ${priBorder(t.priority)} bg-[#F7F3EA] dark:bg-[#20251D]`}>
                              {editingId === t.id ? (
                                <RescheduleInline task={t} dispatch={dispatch} onDone={() => setEditingId(null)} />
                              ) : (
...
function applyQuery(tasks, q) {
  ...
  // simple parser for tag:, project:, priority:, due:today|overdue, completed:true/false, AND/OR
...
function TaskList({ tasks, state, dispatch }) {
  ...
  return (
    <div className="space-y-6">
      {Object.entries(groups).map(([proj, list]) => (
        <div key={proj}>
          <div className="flex items-center gap-2 mb-2 text-sm font-semibold"><ChevronRight className="w-4 h-4"/>{proj} <span className="text-xs text-[#8E8B80]">({list.length})</span></div>
          <div className="space-y-2">
            {list.sort((a,b)=> (a.completed === b.completed ? 0 : a.completed ? 1 : -1)).map(t => (
              <div key={t.id} className={`p-3 rounded-xl border ${priBorder(t.priority)} bg-[#F7F3EA] dark:bg-[#20251D]`}> 
                <div className="flex items-center gap-2">
                  <input type="checkbox" checked={!!t.completed} onChange={()=>dispatch({ type:'TOGGLE_COMPLETE', id: t.id })} className="w-4 h-4"/>
                  <div className={`flex-1 ${t.completed ? 'line-through opacity-60' : ''}`}>
                    ...
                    <div className="text-xs text-[#8E8B80] dark:text-[#A7A99F] flex flex-wrap gap-2 mt-1">
                      {t.due && <span className="">🕒 {format(new Date(t.due), 'EEE, MMM d HH:mm')}</span>}
                  ...
                  <button onClick={()=>setOpenId(openId===t.id?null:t.id)} className="px-2 py-1 rounded-lg border border-zinc-300 dark:border-zinc-700"><ChevronDown className="w-4 h-4"/></button>
                  <button onClick={()=>dispatch({ type:'DELETE_TASK', id: t.id })} className="px-2 py-1 rounded-lg border border-red-300 text-red-600 hover:bg-red-50 dark:border-red-700 dark:hover:bg-red-900/20"><Trash2 className="w-4 h-4"/></button>
                </div>
                {openId === t.id && (
                  <TaskEditor task={t} dispatch={dispatch} />
                )}
              </div>
            ))}
          </div>
...
function TaskEditor({ task, dispatch }) {
  ...
  function save() {
    dispatch({ type:'UPDATE_TASK', id: task.id, payload: {
      title: form.title,
      notes: form.notes,
  ...
  function addSubtask() {
    ...
    dispatch({ type:'UPDATE_TASK', id: task.id, payload: { subtasks } });
  }
...
function RescheduleInline({ task, dispatch, onDone }) {
  const [dt, setDt] = useState(task.due ? format(new Date(task.due), "yyyy-MM-dd'T'HH:mm") : "");
  function save() {
    dispatch({ type:'UPDATE_TASK', id: task.id, payload: { due: dt ? new Date(dt) : null } });
    onDone?.();
  }
...
function ImportExport({ state, dispatch }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <button onClick={exportJSON} className="px-3 py-1 rounded-xl border border-zinc-300 dark:border-zinc-700 hover:bg-zinc-100 dark:hover:bg-zinc-800 flex items-center gap-2"><Upload className="w-4 h-4"/>Export</button>
